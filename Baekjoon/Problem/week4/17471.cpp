...
pair<int, int> dfs(int here, int value){
    visited[here] = 1; 
    pair<int, int> ret = {1, a[here]}; // {노드/컴포넌트의 자식수, 연결된 컴포넌트의 총 인구수}
...
int main(){
    ...
    for(int i = 1; i < (1 << n) - 1; i++){ // 모든 경우의 수에 대해 탐색 (0, n개 뽑는 경우는 제외) 
        ...
        for(int j = 0; j < n; j++){
            if(i & (1 << j)){comp[j + 1] = 1; idx1 = j + 1;} // 켜진 비트에 대해 comp[idx+1] = 1로 색칠함
            else idx2 = j + 1; // 꺼져있으면 comp에 처리할 필요 없음
        }
        // dfs 딱 두번만 돌림
        pair<int, int> comp1 = dfs(idx1, 1); // comp라는 배열이 1로 색칠되어 있으니까
